...
class IntroScenes(QuantumBaseScene):
    def construct(self):
        ...
        # Hook: 7.0s
        # History: 13.0s
        # Classical Impossibility: 13.0s
        # Total for Intro: 33.0s
        ...
        self.play(FadeOut(hook_elements, run_time=0.5)) # Fade out all hook elements
        self.wait(0.1)

        # --- HISTORICAL TIMELINE (7s-20s) ---
        ...
        # --- CLASSICAL IMPOSSIBILITY (20s-33s) ---
        current_scene_duration_target_s2 = 13.0; anim_time_s2 = 0
        scene2_elements = VGroup()
        original_bg_color_scene2 = self.camera.background_color # Should be DARK_BACKGROUND_COLOR already
        self.camera.background_color = ManimColor("#1A1A1A") # Temporary darker BG
        ...
        # Simplified create_subtle_text_bg for this specific use
        ...
        self.camera.background_color = DARK_BACKGROUND_COLOR # Reset BG
        self.wait(0.1)
...
class PeriodFindingAndSuperposition(QuantumBaseScene):
    def construct(self):
        ...
        # --- 1. PERIOD FINDING: THE RHYTHM ON A WHEEL (18.0s) ---
        ...
        # --- 2. SUPERPOSITION: THE QUANTUM MAGIC (10.0s) ---
...
class QFTScene(QuantumBaseScene):
    def construct(self):
        self.setup_scene_defaults()

        # --- 3. QFT: FINDING THE HIDDEN RHYTHM (20.0s) ---
        current_scene_duration_s3_qft = 20.0; anim_time_s3_qft = 0
        scene3_qft_elements = VGroup()
        title_s3_qft = create_title("QFT: Finding the Hidden Rhythm")
        scene3_qft_elements.add(title_s3_qft)
        self.play(Write(title_s3_qft, run_time=0.8)); anim_time_s3_qft += 0.8
        self.wait(0.5); anim_time_s3_qft += 0.5

        input_expl_text = Text("Quantum Input: A Wave Representing 2^x mod 15", font_size=24, color=TEXT_COLOR).next_to(title_s3_qft, DOWN, buff=0.3)
        scene3_qft_elements.add(input_expl_text)
        self.play(Write(input_expl_text), run_time=1.0); anim_time_s3_qft += 1.0

        input_axes = Axes(x_range=[-0.5,15.5,2], y_range=[-1.4,1.4,1], x_length=config.frame_width*0.85, y_length=2.8,
                          axis_config={"include_numbers":False,"color":GRAY,"stroke_width":2}, y_axis_config={"include_tip":False})
        input_axes.next_to(input_expl_text, DOWN, buff=0.3)
        
        x_label_text = MathTex("\\text{Exponent (x)}", font_size=22)
        x_label = input_axes.get_x_axis_label(x_label_text, edge=DOWN, buff=0.2)
        y_label_text = MathTex("\\text{Quantum Amplitude}", font_size=22)
        y_label = input_axes.get_y_axis_label(y_label_text, edge=LEFT, direction=LEFT, buff=0.3)
        scene3_qft_elements.add(input_axes, x_label, y_label)
        self.play(Create(input_axes), Write(x_label), Write(y_label), run_time=1.2); anim_time_s3_qft += 1.2
        self.wait(1.0); anim_time_s3_qft += 1.0

        period_r = 4.0
        def qft_input_wave_func(x):
            base_wave = 0.8 * np.cos(x * TAU / period_r)
            noise_wave = 0.25 * np.sin(x * PI * 1.5)
            return base_wave + noise_wave

        input_wave = input_axes.plot(qft_input_wave_func, x_range=[0, 15], color=PRIMARY_ACCENT_COLOR, stroke_width=3)
        input_wave_glow = input_wave.copy().set_stroke(YELLOW_A, width=12, opacity=0.3)
        input_wave_group = VGroup(input_wave_glow, input_wave)
        scene3_qft_elements.add(input_wave_group)
        self.play(Create(input_wave_group), run_time=1.8); anim_time_s3_qft += 1.8
        self.wait(1.5); anim_time_s3_qft += 1.5

        q_processor_box_s3 = RoundedRectangle(height=2.5, width=3.0, corner_radius=0.2, color=SECONDARY_ACCENT_COLOR, fill_opacity=0.2).move_to(ORIGIN)
        q_processor_label_s3 = Text("QFT", font_size=26).move_to(q_processor_box_s3.get_center())
        
        self.play(
            LaggedStart(FadeOut(input_axes), FadeOut(x_label), FadeOut(y_label), FadeOut(input_expl_text), lag_ratio=0.1),
            input_wave_group.animate.scale(0.3).move_to(q_processor_box_s3.get_center()),
            FadeIn(q_processor_box_s3, q_processor_label_s3), run_time=1.5
        ); anim_time_s3_qft += 1.5
        scene3_qft_elements.add(q_processor_box_s3, q_processor_label_s3)

        center_proc_s3 = q_processor_box_s3.get_center()
        num_qft_waves_s3 = 10; qft_waves_s3 = VGroup()
        for i in range(num_qft_waves_s3):
            wave = ParametricFunction(lambda t,ci=i:center_proc_s3+np.array([t*1.2,0.15*np.sin((ci*0.5+random.uniform(1.8,2.8))*t*PI+random.uniform(0,PI)),0]),
                                   t_range=[-1.0,1.0],stroke_width=random.uniform(1.2,2.2),color=interpolate_color(ManimColor(BLUE_B),ManimColor(TEAL_A),random.random())
                                  ).scale(random.uniform(0.5,0.7)).move_to(center_proc_s3+np.array([random.uniform(-0.1,0.1),random.uniform(-0.1,0.1),0])).rotate(random.uniform(0,TAU))
            qft_waves_s3.add(wave)
        self.play(LaggedStart(*[Create(w) for w in qft_waves_s3], lag_ratio=0.08, run_time=1.0)); anim_time_s3_qft += 1.0
        self.play(LaggedStart(*[w.animate.set_stroke(opacity=0.1).scale(random.uniform(0.7,1.0)) for w in qft_waves_s3],lag_ratio=0.04,run_time=1.0), Rotate(qft_waves_s3,angle=PI*0.65,run_time=1.0,rate_func=rate_functions.smooth)); anim_time_s3_qft += 1.0
        self.play(LaggedStart(*[FadeOut(w,scale=0.1) for w in qft_waves_s3],lag_ratio=0.08), FadeOut(input_wave_group), run_time=0.7); anim_time_s3_qft += 0.7

        output_wave_amplified = input_axes.plot(lambda x: 1.1 * np.cos(x * TAU / 4.0), x_range=[0, 15], color=GRAPH_COLOR, stroke_width=4.5)
        output_wave_amplified.scale(0.3).move_to(q_processor_box_s3.get_center())
        
        self.play(
            FadeIn(output_wave_amplified),
            q_processor_box_s3.animate.set_stroke(color=GRAPH_COLOR),
            run_time=0.5
        ); anim_time_s3_qft += 0.5
        
        original_wave_pos = input_axes.get_center()
        self.play(
            output_wave_amplified.animate.scale(1/0.3).move_to(original_wave_pos),
            FadeOut(q_processor_box_s3), FadeOut(q_processor_label_s3),
            run_time=1.2
        ); anim_time_s3_qft += 1.2
        scene3_qft_elements.add(output_wave_amplified)
        self.wait(1.0); anim_time_s3_qft += 1.0

        spectrum_ax_s3 = Axes(x_range=[0,6.5,1],y_range=[0,1.2,0.5],x_length=config.frame_width*0.6,y_length=2.5,
                              axis_config={"include_numbers":True,"font_size":20,"color":TEXT_COLOR,"decimal_number_config":{"num_decimal_places":0}},
                              x_axis_config={"numbers_to_include":np.arange(1,7,1)},tips=False
                             ).to_edge(DOWN, buff=0.8)
        spectrum_x_label_text = MathTex("\\text{Detected Rhythm Length (Period, r)}", font_size=22)
        spectrum_x_label = spectrum_ax_s3.get_x_axis_label(spectrum_x_label_text, buff=0.25)
        spectrum_y_label_text = MathTex("\\text{Signal Strength}", font_size=22)
        spectrum_y_label = spectrum_ax_s3.get_y_axis_label(spectrum_y_label_text, direction=LEFT, buff=0.3)
        
        self.play(
            FadeOut(output_wave_amplified),
            FadeIn(VGroup(spectrum_ax_s3, spectrum_x_label, spectrum_y_label)),
            run_time=1.0
        ); anim_time_s3_qft += 1.0
        scene3_qft_elements.add(spectrum_ax_s3, spectrum_x_label, spectrum_y_label)

        period_found = 4; bars_s3 = VGroup(); peak_bar_actual_s3 = None
        for r_candidate in range(1, 7):
            bar_height = 0.03 + random.uniform(0,0.04); bar_color = GRAPH_COLOR
            if r_candidate == period_found: bar_height = 1.0; bar_color = PRIMARY_ACCENT_COLOR
            bar = Rectangle(width=0.4,height=bar_height,fill_color=bar_color,fill_opacity=0.9,stroke_width=0).move_to(spectrum_ax_s3.c2p(r_candidate,bar_height/2.0),DOWN)
            bars_s3.add(bar); 
            if r_candidate == period_found: peak_bar_actual_s3 = bar
        
        self.play(LaggedStart(*[GrowFromEdge(b, DOWN) for b in bars_s3], lag_ratio=0.15), run_time=1.2); anim_time_s3_qft += 1.2
        scene3_qft_elements.add(bars_s3)
        
        if peak_bar_actual_s3 is not None:
            period_label_s3 = Text("PERIOD = 4!", font_size=34, color=PRIMARY_ACCENT_COLOR, weight=BOLD).next_to(peak_bar_actual_s3, UP, buff=0.25)
            self.play(peak_bar_actual_s3.animate.set_fill(YELLOW_A).scale(1.2,about_edge=DOWN), Write(period_label_s3), run_time=0.8)
            self.play(peak_bar_actual_s3.animate.set_fill(PRIMARY_ACCENT_COLOR).scale(1/1.2,about_edge=DOWN), run_time=0.4)
            anim_time_s3_qft += (0.8+0.4)
            scene3_qft_elements.add(period_label_s3)
        
        self.wait(max(0.01, current_scene_duration_s3_qft - anim_time_s3_qft - 0.5))
        self.play(FadeOut(scene3_qft_elements, run_time=0.5))
        self.wait(0.1)
...
class OutroScene(QuantumBaseScene):
    def construct(self):
        self.setup_scene_defaults()
        # --- TIMING ---
        # CTA: 5.0s (Target: 80s - 85s)
        # Credits: 5.0s (Target: 85s - 90s)
        # Total for Outro: 10.0s

        # --- CALL TO ACTION (5.0s) ---
        current_scene_duration_cta = 5.0; anim_time_cta = 0
        cta_elements = VGroup()

        cta1 = Text("Quantum finds hidden patterns,", font_size=36, color=TEXT_COLOR, t2w={'hidden patterns': BOLD}).move_to(UP*0.8)
        cta2 = Text("solving great challenges.", font_size=36, color=TEXT_COLOR, t2w={'great challenges': BOLD}).next_to(cta1, DOWN, buff=0.3)
        cta3 = Text("What secrets will YOU unlock?", font_size=48, color=PRIMARY_ACCENT_COLOR, weight=BOLD).next_to(cta2, DOWN, buff=0.8)
        ...
        sparkle_duration = 1.3 # Fit into 5s total
        sparkle_anims_cta = []
        for _ in range(35):
